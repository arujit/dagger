...
//            Table table = tableEnvironment.fromDataStream(stream, $("user_action_time").rowtime(), $("user_name"), $("data"));
//            Table table = tableEnvironment.fromDataStream(streamInfo.getDataStream(),$("rowtime").rowtime());
//            - Long watermarkDelay = configuration.getLong(FLINK_WATERMARK_DELAY_MS_KEY, FLINK_WATERMARK_DELAY_MS_DEFAULT);
//            WatermarkStrategy ws = new BoundedOutOfOrderTimestamps(watermarkDelay);
//             - Duration maxOutOfOrderness = Duration.ofSeconds(20);
//            TimestampExtractor existingField = new ExistingField(rowTimeAttributeName);
//            WatermarkStrategy<Row> ws = WatermarkStrategy.
//                    <Row>forBoundedOutOfOrderness(maxOutOfOrderness);
//                    .withTimestampAssigner((element, recordTimestamp) -> (long) element.getField("rowtime"));

//            SingleOutputStreamOperator<Row> rowSingleOutputStreamOperator = streamInfo.getDataStream().assignTimestampsAndWatermarks(ws);
//            Table table = tableEnvironment.fromDataStream(streamInfo.getDataStream(), $("user_name"), $("data"), $("user_action_time").rowtime());
